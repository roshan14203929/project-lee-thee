...
baseline. Each of the three top-level folders shares the same shape:
`general-rules.md` (bare delivery/process rules), `coding/*.md`, `qa/*.md`.
`guidelines/builder.md` and `guidelines/extractor.md` sit outside this
structure — they're agent-role files, not channel content.

Role and platform are two independent axes. **Role** picks one file per agent
(`builder.md`, `guidelines/global/qa/ui-qa.md`, …). **Platform** picks the
...
`general-rules.md`) the same way. The two rulesets are mutually exclusive, so
`new-run` refuses to start until a platform is set, and a role-scoped read
without one opens with an explicit warning. The unscoped read remains the full
archival record of every file under `guidelines/`.

- **`global/general-rules.md`** — the master rulebook: extraction rules, run
  immutability, numeric QA gates (max 3 repair rounds, 5% full-page pixel diff
  ceiling, 12% localized band ceiling, 0.10pt regression tolerance, 0.20pt
  required improvement after round 1), content/UI/accessibility fidelity
  rules, and release rules (only the orchestrator creates releases, exact
  byte-for-byte copy).
- **`global/coding/{html,css,assets-media}.md`** — the channel-agnostic HTML/CSS
  baseline: document structure, semantic HTML, accessibility, BEM + `cst-`
  naming, design tokens, CSS architecture/hygiene, image/asset rules,
  character-encoding preflight. Channel folders add only deltas (e.g. font-size
  unit) on top of this.
